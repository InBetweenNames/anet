...
typedef struct {
	tcapw_uid_t uid;		/* tcapw_UID_NONE if login failed */
	dp_result_t reason;    /* dp_RES_OK or reason for failure */
} PACK tserv_packet_validate_t;

/* Packet sent from server to client after server gets pwchange. */
typedef struct {
	dp_result_t reason;    /* dp_RES_OK or reason for failure */
} PACK tserv_packet_pwvalidate_t;
