...
typedef struct {
	unsigned short key;/* tag for this data element */
	short flags;		/* see #defines above */
	size_t len;		/* length of user data */
	long crc;			/* checksum */
	char type[pv_VAR_TYPE_LEN];/* MS-DOS filename extension, null terminated */
	size_t offset;		/* if not yet fully transferred, next byte to transfer */
	void *buf;			/* pointer to malloc'd user data */
} PACK pv_var_t;

/* A peer and all its variables */
typedef struct {
	assoctab_t *vars;	/* current variables */
	int dirty;			/* boolean set by pv_set, cleared by pv_update. */
	size_t allocated;	/* bytes allocated for incoming.buf */
	pv_var_t incoming;	/* incoming variable held here until complete */
} PACK pv_peer_t;
...
typedef struct {
	...
	size_t len;			/* length of user data */
	long crc;				/* checksum */
	char type[pv_VAR_TYPE_LEN];	/* MS-DOS filename extension or 0 */
	dpid_t id;
} PACK pv_playerData_initial_packet_t;
...
typedef struct {
	dpid_t id;				/* wish we didn't have to send that */
} PACK pv_playerData_body_packet_t;
...
typedef struct {
	 ...
	 */
	dp_result_t err;
} PACK pv_playerData_reply_packet_t;
